...
description: Google Play Store 앱 페이지에서 공식 아이콘 이미지를 다운로드하고, 둥근 사각형으로 clip 하여 Windows exe 용 multi-size `.ico` 로 변환. 앱 패키지 ID 또는 아이콘 URL 직접 지정 가능. Android 13 / iOS 스타일 22% corner radius 기본. Use when the user wants to refresh the app's Windows icon from the official Play Store listing — examples "아이콘 새로 받아줘", "아이콘 갱신", "refresh app icon", "make rounded icon".
---
